...
export default function HelpCentersScreen() {
  ...
  return (
    <div className={`relative flex min-h-screen flex-1 w-full max-w-[430px] lg:max-w-3xl mx-auto flex-col bg-[#f6f7f8] dark:bg-slate-900 font-display text-slate-900 dark:text-white ${view === "list" ? "pb-mobile-tab lg:pb-8" : ""}`}>
      {/* Header */}
      ...
      {view === "list" && (
        <div className="flex flex-1 flex-col gap-3 px-4 py-4 min-h-0">
          {isLoading ? (
            <div className="flex flex-1 w-full min-h-[45dvh] items-center justify-center">
      ...
      {/* Vista MAPA */}
      {view === "map" && (
        <div className="flex flex-1 flex-col min-h-0" style={{ minHeight: "calc(100dvh - 57px)" }}>
          {isLoading ? (
            ...
            <MapContainer
              center={mapCenter}
              zoom={12}
              style={{ flex: 1, height: "100%", minHeight: "calc(100dvh - 57px)" }}
              zoomControl={true}
            >
              <TileLayer
                key={theme}
